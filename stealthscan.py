...
def stealthScanner(IPs, ports, ttl, scrPort, timeOut):
    ...
    for tgt in IPs:
        if platform == 'linux':
            p = Popen(["iptables", "-A", "OUTPUT", "-p", "tcp", "--tcp-flags", "RST", "RST", "-d", str(tgt), "-j" "DROP"])
        ...
        for tgtPort in ports:
            ...
            if(str(type(stealth_scan_resp))=="<class 'NoneType'>"):
                ...
                if(stealth_scan_resp.getlayer(TCP).flags == 0x12):
                    send_rst = sr(IP(dst=str(tgt),ttl=ttl)/TCP(sport=scrPort,dport=tgtPort,flags="R"),timeout=timeOut, verbose=False)
        ...
        if platform == 'linux':
            p = Popen(["iptables", "-D", "OUTPUT", "-p", "tcp", "--tcp-flags", "RST", "RST", "-d", str(tgt), "-j" "DROP"])
    return scanResults
...
def main():
    parser = ArgumentParser(description = "Basic syn scanner Syn->Syn/Ack->RST")
    parser.add_argument("IP", nargs="+", help="Tgt IP(s)", type=str)
    parser.add_argument('-p', '--port', nargs='*', help='Tgt ports', type=int, default="80")
    parser.add_argument('-t', '--ttl', help='TTL', type=int, default="128")
    parser.add_argument('-sp', '--srcPort', help='Src Port', type=int, default="0")
    parser.add_argument('-to', '--timeOut', help='Time Out', type=int, default="1")
    args = parser.parse_args()

    returnList = stealthScanner(IPtoList(args.IP), args.port, args.ttl, args.srcPort, args.timeOut)
